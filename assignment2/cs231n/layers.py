...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            mask = x[:, :, i*stride:i*stride+pool_h, j*stride:j*stride+pool_w] == np.max(x[:, :, i*stride:i*stride+pool_h, j*stride:j*stride+pool_w], axis = (2,3))[:,:,None,None] 
            dx[:, :, i*stride:i*stride+pool_h, j*stride:j*stride+pool_w] += mask * (dout[:,:,i,j])[:,:,None,None]  
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
